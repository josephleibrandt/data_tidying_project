...
We save the two datasets locally just in case there is a future problem accessing them on the web.
...
This is where we work with the two datasets so that they are in a usable format and can be merged into one dataframe.
...
Here is where we plot the retail sales data, the department sales data, and the combined data (all on separate charts).
